...
def check_buffer_from_function(module_path, function_name, what, ascii_selector, *args, **kwargs):
    ...
    if ascii_selector and any(ord(c) >= 0x80 for c in str(result)):
        error('Your {} in {} contains non-ascii bytes\n{}'.format(
              what, module_path, repr(result)))
        return False
...
def check_get_decoder(module_path):
    return check_buffer_from_function(module_path, 'get_decoder', 'decoder',
        (lambda str: str),
        indices=[0, 1, 6, 313]
    )
